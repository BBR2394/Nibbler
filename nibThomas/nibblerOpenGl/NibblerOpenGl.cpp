...
void NibblerOpenGl::createWin(int x, int y)
{
...
int NibblerOpenGl::init(int x, int y)
{
    if (x > 80 || y > 40)
        {
...
int NibblerOpenGl::refreshScreen()
{
    SDL_Surface     *pion;
    SDL_Rect        pos;
...
int NibblerOpenGl::drawGame(int x, int y, t_type tp)
{
    SDL_Surface     *pion;
    SDL_Surface     *gomme;
    SDL_Rect        pos;
...
int NibblerOpenGl::timeToWait(int ms)
{
    SDL_Delay(ms);
    return (0);
}

int NibblerOpenGl::printSomething(t_texte msgToDisplay, int scr)
{
    SDL_Surface *text;
    SDL_Rect    position;
    std::string score;
